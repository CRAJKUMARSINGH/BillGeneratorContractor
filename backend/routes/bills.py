...
def _generate_documents(job_id: str, req: GenerateRequest):
    ...
    try:
        ...
        # process_bill() reads:
        #   header  → ws_wo.iloc[:19, :7]
        #   items   → ws_wo.iloc[i, 4] = rate,  ws_bq.iloc[i, 3] = qty
        # All DataFrames must share the same 7-column schema so concat aligns.

        NCOLS = 7  # columns 0-6 used by process_bill

        wo_rows = []
        bq_rows = []
        for item in req.billItems:
            # col: 0=sno, 1=desc, 2=unit, 3=qty_upto, 4=rate, 5=amount, 6=remark
            wo_rows.append([item.itemNo, item.description, item.unit,
                            item.quantityUpto, item.rate, item.amount, ""])
            # col: 0=sno, 1=desc, 2=unit, 3=qty_since, 4=rate, 5=amount, 6=remark
            bq_rows.append([item.itemNo, item.description, item.unit,
                            item.quantitySince, item.rate, item.amount, ""])

        ws_wo = pd.DataFrame(wo_rows, columns=range(NCOLS))
        ws_bq = pd.DataFrame(bq_rows, columns=range(NCOLS))

        extra_rows = []
        for ei in req.extraItems:
            extra_rows.append([
                ei.itemNo, ei.bsr, ei.description,
                ei.quantity, ei.unit, ei.rate, ei.amount
            ])
        ws_extra = pd.DataFrame(extra_rows, columns=range(NCOLS)) if extra_rows else pd.DataFrame()

        # ── Inject title header rows ──────────────────────────────────────────
        # Pad each header row to NCOLS so concat doesn't create extra columns.
        header_rows = [[k, v] + [""] * (NCOLS - 2) for k, v in req.titleData.items()]
        while len(header_rows) < 19:
            header_rows.append([""] * NCOLS)
        header_df = pd.DataFrame(header_rows[:19], columns=range(NCOLS))
        spacer    = pd.DataFrame([[""] * NCOLS],   columns=range(NCOLS))  # row 20
