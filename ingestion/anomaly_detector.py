...
def detect_anomalies(current_features: Dict[str, float]) -> List[str]:
    ...
    # Save the current features to history to train the model over time
    historical.append(current_features)
    _save_historical_features(historical)
    
    # Need at least 3 historical bills to calculate meaningful standard deviation
    if len(historical) < 3:
        return warnings

    df = pd.DataFrame(historical)
    
    # Calculate Z-scores for finding outliers (> 2 standard deviations)
    for feature in ["total_amount", "max_quantity", "avg_rate", "item_count"]:
        if feature in df.columns:
            ...
            if std > 0:
                ...
                if z_score > 2.0:
                    warnings.append(
                        f"Anomaly Detected: {feature.replace('_', ' ').title()} ({current_val:.2f}) is usually outside historical norms!"
                    )
    return warnings
